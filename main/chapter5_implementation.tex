...
We manage all hyperparameters through Python dataclasses with type safety and serialization support. The genome class converts genetic encodings to network configurations via a \texttt{to\_config()} method, separating evolvable architectural choices from fixed training parameters.
...
This chapter detailed implementation choices translating the conceptual approach into working software: Python/PyTorch for rapid prototyping, 8-dimensional state and 25-dimensional action features with $7 \times 7$ heightmap patches, genome-driven architecture flexibility, potential-based reward shaping (utilization + 0.3·EMS quality), Double DQN with 200K replay buffer and 15-step returns, and evolutionary search with population 20 over 10 generations optimizing multi-objective fitness (70\% utilization, 20\% bins, 10\% complexity).
