...
The genome class converts genetic encodings to network configurations via a \texttt{to\_dqn\_config()} method, separating evolvable architectural choices from fixed training parameters.
...
We use a 200K-transition circular buffer with 1,000-step warmup. Double DQN maintains online and target networks, with hard target updates every 500 steps. The online network selects actions while the target network evaluates them.
...
For variable action counts (10-500+), we pad action features to 128 with boolean masks and set invalid Q-values to $-\infty$, enabling GPU batch processing.
...
\begin{itemize}
    \item Hidden dimensions: base 256, mutated by $\times 2$ or $\div 2$, range [64, 1024]
    \item Encoder layers: base 2, mutated by $\times 2$ or $\div 2$, range [1, 8]
    \item Head hidden size: base 256, mutated by $\times 2$ or $\div 2$, range [64, 1024]
    \item Attention heads: base 4, mutated by $\times 2$ or $\div 2$, range [2, 16]
    \item Inducing points: base 32, mutated by $\times 2$ or $\div 2$, range [8, 128]
    \item Patch size: base 7, mutated by $\times 2$ or $\div 2$, range [3, 15]
\end{itemize}

This multiplicative mutation scheme enables large architectural changes while maintaining reasonable values. Each gene starts at its base value and mutation randomly multiplies or divides by 2, clamped to the specified range.
...
\begin{itemize}
    ...
    \item Dropout rate: 0.0, 0.05, 0.1, 0.15, 0.2
\end{itemize}
