...
class PackingEnv:
    def __init__(self, W=40, D=40, H=40, items: List[Tuple[int,int,int]] = None, max_actions: int = 128, topk_eps: int = 150, seed: int = 0):
...
def train_pack_dqn(episodes=50, W=40, D=40, H=40, n_items=50, seed=42, max_actions=128, topk_eps=150):
    env = PackingEnv(W,D,H, items=make_items(n=n_items, seed=seed), max_actions=max_actions, topk_eps=topk_eps, seed=seed)
    obs = env.reset()
    OBS_DIM = obs.shape[0]
    cfg = DQNConfig(
        obs_dim=OBS_DIM,
