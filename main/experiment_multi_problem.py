"""
Experiment: Multi-Problem Training with Target Evaluation

This experiment trains a neural architecture on multiple different problems
and then evaluates on a separate target problem. This tests generalization
across diverse problem instances.

Key features:
1. Train on multiple specified problems (training datasets)
2. Evolve architecture using multi-problem fitness aggregation
3. Train final model on the evolved architecture
4. Evaluate only on the target test problem

Supports:
- Arbitrary training and test dataset specification
