"""
Multi-Bin Packing Environment with Neurogenesis

This version uses genetic algorithms to evolve the DQN architecture before training.

Workflow:
1. EVOLUTION PHASE: Evolve network architecture using GA (5-15 generations)
2. TRAINING PHASE: Train best evolved architecture normally
3. VISUALIZATION: Show results and architecture stats

The environment and problem remain identical - only the network architecture is evolved.
"""
...
# Import neurogenesis components
from dqn_core.dqn_neurogenesis_fc import FCNeurogenesisController, FCGenome
from dqn_core.dqn import DQNConfig
# Import evolvable agent if available, otherwise use base
...
def train_multibin_pack_with_neurogenesis(
    problem_path: str,
    episodes: int = 1000,
    seed: int = 42,
    max_actions: int = 128,
    ...
    eval_episodes_per_genome: int = 5,
    eval_steps_per_genome: int = 200,
    device: str = "cpu",
):
    """
    Train multi-bin packing with neurogenesis (architecture evolution).
    
    Workflow:
    1. PHASE 1: Evolve network architecture (5-15 generations)
    2. PHASE 2: Train best architecture normally (like original)
    3. PHASE 3: Visualize results
    
    New parameters:
        evolution_generations: Number of GA generations (5-15 recommended)
        population_size: Size of genome population (10-20 recommended)
        mutation_rate: Mutation probability (0.1-0.2 recommended)
        eval_episodes_per_genome: Quick eval episodes during evolution
        eval_steps_per_genome: Training steps per genome evaluation
    """
    
    print("="*80)
    print("MULTI-BIN PACKING DQN WITH NEUROGENESIS")
    print("="*80)
    print(f"Problem: {problem_path}")
    print(f"Evolution: {evolution_generations} generations, pop_size={population_size}")
    ...
    print(f"  Bin size: {problem.bin_dimensions}")
    ...
    print("="*80)
    print("PHASE 1: ARCHITECTURE EVOLUTION")
    print("="*80)
    
    # Create base config (your current architecture as baseline)
    ...
    print(base_genome)
    print(f"Parameters: {base_genome.get_param_count(obs_dim, action_feat_dim):,}")
    print()
    
    # Create neurogenesis controller
    ...
    # Initialize population (seeded with base architecture)
    controller.initialize_from_base()
    print(f"Population initialized: {len(controller.pop_manager.population)} genomes")
    print(f"Seeded with base architecture for comparison\n")
    
    # Define evaluation function for genomes
    def evaluate_genome(genome: FCGenome, agent) -> float:
        """
        Quick evaluation: train briefly and measure performance.
        
        Fitness = avg_utilization - complexity_penalty
        """
        # Quick training
        for _ in range(eval_steps_per_genome):
            obs = env.reset(items=items.copy())
            done = False
            step_count = 0
            
            while not done:  # Max 50 steps per episode
                actions, mask_short = env.action_space()
                feats = build_action_features(env, actions) if actions else np.zeros((0, ACTION_FEAT_DIM), np.float32)
                ...
                act = None if (act_idx is None or not actions) else actions[act_idx]
                ...
                n_feats = build_action_features(env, n_actions) if n_actions else np.zeros((0, ACTION_FEAT_DIM), np.float32)
                nextF, nextM = pad_feats_mask(n_feats, n_mask_short if n_mask_short.shape[0] > 0 else np.zeros((0,), np.float32), max_actions)
        ...
        # Evaluation phase
        total_util = 0.0
        for _ in range(eval_episodes_per_genome):
            obs = env.reset(items=items.copy())
            done = False
        ...
        avg_util = total_util / eval_episodes_per_genome
        
        # Fitness = performance - complexity penalty
        complexity = genome.complexity_penalty(weight=0.0001)
        fitness = avg_util - complexity
    ...
    print(best_genome)
    print(f"Parameters: {best_genome.get_param_count(obs_dim, action_feat_dim):,}")
    print(f"Fitness: {best_genome.fitness:.4f}")
    
    # Compare with base
    param_reduction = (1 - best_genome.get_param_count(obs_dim, action_feat_dim) / 
                      base_genome.get_param_count(obs_dim, action_feat_dim)) * 100
    print(f"Parameter change vs base: {param_reduction:+.1f}%")
    print("="*80 + "\n")
    ...
    print("="*80)
    print("PHASE 2: FULL TRAINING WITH EVOLVED ARCHITECTURE")
    print("="*80)
    
    # Build agent from best genome
    agent = controller.build_agent_from_genome(best_genome)
    
    print(f"Agent created with evolved architecture")
    print(f"Starting full training for {episodes} episodes...\n")
    ...
    print(f"  {best_genome}")
    print(f"  Parameters: {best_genome.get_param_count(obs_dim, action_feat_dim):,}")
    print(f"\nImprovement: {param_reduction:+.1f}% parameters")
    print(f"{'='*80}\n")
    
    # Save model and genome
    if save_path:
        ...
        # Save genome
        genome_path = save_path.replace('.pth', '_genome.pkl')
        controller.save_best_genome(genome_path)
        print(f"Genome saved to: {genome_path}\n")
    ...
    for i, bin in enumerate(bins_to_visualize):
        if len(bin.placed) > 0:
            ...
            # Save with extreme points
            bin.plot3d(
                title=f"Bin {i+1}/{env.max_bins} ({len(bin.placed)} items, util:{bin_util:.3f}) [EVOLVED]",
                save_path=f"output_data/evolved_bin_{i+1}_with_eps.png",
                show=False
            )
            ...
            bin.plot3d_filled(
                title=f"Bin {i+1}/{env.max_bins} ({len(bin.placed)} items, util:{bin_util:.3f}) [EVOLVED]",
                save_path=f"output_data/evolved_bin_{i+1}_filled.png",
                show=False
            )
    
    return agent, env, best_solution, best_genome, controller
...
if __name__ == "__main__":
    ...
    # Train with neurogenesis
    agent, env, best_solution, best_genome, controller = train_multibin_pack_with_neurogenesis(
        ...
        num_train_steps=1,
        log_interval=10,
        save_path=f"neurogenesis_multibin_dqn_{problem.replace('.txt','')}.pth",
        # Neurogenesis parameters
        evolution_generations=10,      # 10 generations of evolution
        population_size=15,            # 15 genomes in population
        mutation_rate=0.15,            # 15% mutation rate
        eval_episodes_per_genome=20,    # 5 episodes to evaluate each genome
        eval_steps_per_genome=2000,     # 200 training steps per genome
        device="cpu"
    )
    ...
    print(f"\n{'='*80}")
    print(f"FINAL STATISTICS (BEST SOLUTION WITH EVOLVED ARCHITECTURE):")
    print(f"{'='*80}")
    ...
    print(f"{'='*80}")
    print(f"\nEvolution statistics saved to: output_data/")
    print(f"Visualizations saved to: output_data/evolved_bin_*")
